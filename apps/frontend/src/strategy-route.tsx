  ...
  return <>
    <section className="running-strategies terminal-panel">
      ...
      {showingPositions ? <>
        ...
        <div className={`running-table ${showingHistory ? 'historical' : ''}`}><div className="running-head">{showingHistory && <span>{t('Start time')}</span>}<span>{t('Strategy')}</span><span>{t('Market name')}</span><span>{t('Exchanges')}</span><span>{t('Entry / Exit')}</span>{showingHistory && <span>{t('Net trade PnL')}</span>}<span>{t('Size limits')}</span><span>{t('Method')}</span><span>{t('Execution log')}</span><span>{t(showingHistory ? 'Duration' : 'Runtime')}</span><span>{t('Status')}</span>{!showingHistory && <span />}</div>{visibleStrategies.map((strategy) => {
      ...
      return <div className="running-row" key={strategy.id}>
        ...
        {showingHistory && <span className={`strategy-realized-pnl ${Number(strategy.netRealizedPnl) >= 0 ? 'positive' : 'negative'}`} title={t('After trading fees; funding excluded')}><strong>{Number(strategy.netRealizedPnl) >= 0 ? '+' : ''}{Number(strategy.netRealizedPnl).toFixed(2)}</strong><small>{t('USD est.')} · {t('fees')} {Number(strategy.tradingFees).toFixed(2)} · {t('funding excl.')}</small></span>}
